...
구글 피트니스의 경우, 전체적으로 RESTful API 컨셉을 가진다. google fitness의 정보를 필요로 하는 어플리케이션에서 Google Cloud Platform을 이용한 통신을 사용한다. 어플리케이션 간의 직접적인 통신이 아니라 미들웨어를 거쳐 진행되기에 GCP의 인증 절차가 필수적이다.
 ...
 1. 구글 피트니스를 실행하기 위해서는 인터넷 연결이 필수이다."네트워크 상태를 확인해주세요"
    - 인터넷 연결 상태 확인 필요
...
👍 몸무게           | O    | O     |
👍 **걸음 수**      | O    | O    | 
👍 칼로리 소모량     | O    |  O   | 단위는 kcal
👍 체온            | O    | O    | 단위는 degree
👍 심박수           | O   |   O  | 
👍 이동거리         | O    |   O   | Android에서 ```ACCESS_FINE_LOCATION``` 접근 권한 필요, 단위는 미터
👍 수분량           | O    |   O   |
👍 수면 시간        | O    | O     | 
최대 및 최소 속도    | O    |      |
이동 시간          | O    |      |
이동 범위          | O    |      | bounding box형태의 데이터

#### TODO LIST
- [ ] 각 타입별 데이터 없는 경우 반환값
- [ ] 스마트 워치 연동 데이터 없는 경우 반환값
- [ ] 피트니스 초기 설정(키, 몸무게)없는 경우 칼로리, 키, 몸무게 반환값
- [ ] 상황별 예외 대응
